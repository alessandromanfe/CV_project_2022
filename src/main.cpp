...
void faceDetection(Mat frame)
{
    ...
    if(pos1!=-1)
    {
        ...
        Rect r1 = Rect(faces[pos].x + eyes[pos1].x, faces[pos].y + eyes[pos1].y, eyes[pos1].width, eyes[pos1].height);
        reyes.push_back(r1);
    }
    if(pos2!=-1)
    {    
        ...
        Rect r2 = Rect(faces[pos].x + eyes[pos2].x, faces[pos].y + eyes[pos2].y, eyes[pos2].width, eyes[pos2].height);
        reyes.push_back(r2);
    }

    for (Rect r : reyes) {
        vector<int> coordVal = dm::printIris(h_frame, &h_frame, r);
    }
